...
function ReviewPage(props) {
    ...
    let arrayReviews = props.arrayReviews;
    const reviewId = props.match.params.id;
    const thisReview = arrayReviews.find((review) => review.id === reviewId);
    const comments = thisReview.comments;

    let oneComment = {};

    const queryReview = async () => {
        const urlThisReview = "http://localhost:4000/api/reviews/"+reviewId;
        try {
    ...
    const handleSubmit = async (e) => {
        ...
        if (username.trim() === "" || content.trim() === "") {
            ...
            try {
                ...
                const result = await fetch(urlCategories, header);
                ...
                switch (result.status) {
                    case 201:
                        ...
                        props.queryReviews();
                        props.history.push("/reviews/" + reviewId);
                        setUsername("");
                        setContent("");
                        break;
                    case 403:
